...
userRouter.post('/register', async (request, response) => {
    ...
    if (!(username && password && name)) return response.status(400).send({ "error": "Missing required data "});
    ...
    try {
        ...
        if (result) return response.status(409).send({ "error": "Username already exists" })
        ...
        const token = JWT.sign(userForToken, config.SECRET, { expiresIn: '1hr' });

        return response.status(200).send({ token, username: user.username, name: user.name })
        
    } catch (error) {
        console.log(error)
        return response.status(404).send({ "error": "Request resulted in an error "})
    }
